...
Finally, prior to generating the taxonomy, we need to define a set of variable genes and build a 2-dimensional representation of the data.  To do this we use binary genes and the `umap` R library, respectively, although other options could also be used.  
  
```{r create Seurat object}
## Get top 1500 genes by a beta (binary) score
binary.genes <- top_binary_genes(counts,clusters,1500)

## Calculate a umap based on these binary genes.
umap.coords <- umap(logCPM(counts)[variable_genes,])$layout
```
