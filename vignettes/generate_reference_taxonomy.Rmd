...
```{r create Seurat object, fig.height=8, fig.width=10}
...
Finally, we read this information to a compact annData format compatible with both R and python.  **This annData object is the input file for all mapping algorithms in this package, and a to-be-created parallel python package.**.  
  
```{r create annData object}
AIT.anndata <- loadTaxonomy(refFolder, 
                            nGenes=2000, 
                            hGenes=NULL) # Can use this if there is specific set of variable genes to provide
```
