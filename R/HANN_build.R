...
build_train_list_WB21.1 <- function(pre.train.list=NA, query.genes=NA, WBDIR, TaxDir, prefix="", nlevel=4, TaxFN, rm.cl=c())
{
   ...
   load(file.path(TaxDir, "ensembl_ncbi.markers.rda")) # pooled
   if (is.na(query.genes)) query.genes = ensembl_ncbi.markers

   if (is.list(pre.train.list)) {
      select.markers = intersect(pre.train.list$all.markers, query.genes)
   } else {
      select.markers = intersect(ensembl_ncbi.markers, query.genes)
   }
